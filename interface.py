...
def criaJanelaHist():
    ...
    janelaHist = tk.Tk()
    janelaHist.tittle("Histórico Transações")
    janelaHist.geometry('400x400')

    janelaHist = tk.Label(janelaHist, text="")
    janelaHist.pack(padx=200, pady=200)
...
def telaAdmin(x):
    ...
    x.destroy()
    janela = tk.Tk()
    janela.geometry("470x340+525+340")
    janela.title("Controle de Estoque - Unibra (Administrador)")

    #----------------------LAYOUT DOS BOTOES--------------------------
    botaoTransacao = tk.Button(janela, text="Registrar Transação", 
                               width="50", height="3", command=criaJanelaTransacao)
    botaoTransacao.place(x=20,y=20)

    botaoCadastro = tk.Button(janela, text="Gerenciamento de Usuário", 
                              width="50", height="3", command=criaJanelaGerenciamento)
    botaoCadastro.place(x=20,y=80)

    botaoPesquisa = tk.Button(janela, text="Pesquisar Produto", 
                              width="50", height="3", command=criaJanelaPesquisaProduto)
    botaoPesquisa.place(x=20,y=140)

    botaoHistorico = tk.Button(janela, text="Histórico de Transações", 
                               width="50", height="3", command=criaJanelaHist)
    botaoHistorico.place(x=20,y=200)

    botaoEstoque = tk.Button(janela, text="Checar Estoque", 
                             width="50", height="3")
